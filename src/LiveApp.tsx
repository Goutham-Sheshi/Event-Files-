...
function ResourceCard({resource}:{resource:Resource}){
  ...
  return <div data-resource-id={resource.id} data-resource-tags={JSON.stringify(resource.tags||[])} data-resource-type={resource.type} data-resource-description={resource.description||''} className="group bg-white border border-[var(--line-soft)] rounded-xl overflow-hidden flex flex-col hover:shadow-lg transition-shadow cursor-pointer" onClick={()=>{if(resource.sourceUrl){openViewer(resource.sourceUrl,resource.title,resource.id,(resource.tags||[]),resource.type,resource.description||'',resource.contentStatus||'Active',resource.version||'v1.0')}}}><div className="h-40 bg-[var(--canvas-deep)] flex items-center justify-center overflow-hidden relative">{resource.thumbnail?<img src={resource.thumbnail} alt={resource.title} className="w-full h-full object-cover" style={{objectPosition:'left top'}} loading="lazy" onError={e=>{e.currentTarget.style.display='none'}}/>:<FileIcon/>}{isVideo&&<div className="absolute inset-0 flex items-center justify-center bg-black/10"><span className="w-10 h-10 rounded-full bg-white/90 flex items-center justify-center"><PlayIcon/></span></div>}{resource.fileFormat&&<span className="absolute top-2 right-2 bg-black/50 text-white rounded px-2 py-1 text-[9px] font-mono font-bold uppercase">{resource.fileFormat}</span>}{isOfficial&&<span className="absolute top-2 left-2 bg-[var(--primary)] text-white rounded-full px-2 py-0.5 text-[9px] font-bold shadow-xs">Official ✓</span>}</div><div className="p-3.5 flex flex-col gap-2 flex-1"><div className="line-clamp-2 text-[20px] leading-[1.15] font-semibold min-h-[46px]">{resource.title}</div>{tags.length>0&&<div className="flex flex-wrap gap-1.5">{tags.map(tag=><span key={tag} className="resource-tag">{tag}</span>)}</div>}<div className="flex items-center gap-2"><ProductBadge product={p}/>{resource.version&&<span className="bg-[var(--canvas-deep)] text-[var(--ink-45)] px-1.5 py-0.2 rounded text-[9px] font-mono font-bold border border-[var(--line-soft)]">{resource.version}</span>}{resource.fileSize&&<span className="text-[10px] text-[var(--ink-45)]">{resource.fileSize}</span>}</div><div className="mt-auto flex justify-between items-center text-[11px]"><span className="text-[var(--ink-45)]">{resource.viewCount||0} views</span>{resource.sourceUrl&&<button onClick={(e)=>{e.stopPropagation();if(isVideo){window.open(resource.sourceUrl!,'_blank','noreferrer')}else{triggerDirectDownload(resource.sourceUrl!,resource.title)}}} className="font-semibold text-[var(--ink)] hover:underline border-0 bg-transparent p-0 cursor-pointer">{isVideo?'Open Video':'Download'}</button>}</div></div></div>
}
    ...
    <div className="px-8 py-6 max-w-[1400px] space-y-9">
      ...
      <section>
        <div className="flex items-end justify-between gap-4 mb-4">
          ...
          {sheshiResources.length>0&&<button onClick={onSheshi} className="text-[11px] font-semibold text-[var(--primary)] whitespace-nowrap">View all</button>}
        </div>
      ...
      <div>
        {resource.sourceUrl&&(
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 text-[11px] font-semibold transition-colors"
          >
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between bg-white p-3 rounded-2xl border border-[var(--line-soft)] shadow-sm">
        <div className="relative flex-1">
          <input
            type="text"
            placeholder="Search files by name, format, or tags..."
            value={search}
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex items-center">
            ...
            <span className="absolute left-2.5 text-[var(--ink-45)] pointer-events-none"><SortIcon/></span>
            <span className="absolute right-2.5 text-[var(--ink-45)] pointer-events-none text-[10px]">▼</span>
          </div>

          <div className="flex border border-[var(--line-soft)] rounded-xl overflow-hidden p-0.5 bg-[var(--canvas-deep)]">
            <button
              onClick={()=>setViewMode('grid')}
              className={`p-1.5 rounded-lg transition-colors ${viewMode==='grid'?'bg-white text-[var(--primary)] shadow-sm':'text-[var(--ink-45)]'}`}
              title="Grid View"
            >
              <GridViewIcon/>
            </button>
            <button
              ...
              className={`p-1.5 rounded-lg transition-colors ${viewMode==='list'?'bg-white text-[var(--primary)] shadow-sm':'text-[var(--ink-45)]'}`}
      ...
      <div className="flex gap-2 flex-wrap items-center">
        {[
          {id:'all',label:'All Files',count:typeCounts.all},
          {id:'video',label:'Videos',count:typeCounts.video},
          {id:'brand_assets',label:'Logos & Brand Assets',count:typeCounts.logo+typeCounts.brochure},
          {id:'document',label:'Documents',count:typeCounts.document},
          {id:'other',label:'Other',count:typeCounts.other},
        ].map(chip=>(
          <button
            key={chip.id}
            onClick={()=>setTypeFilter(chip.id)}
            className={`px-3 py-1.5 rounded-full text-[11.5px] font-semibold transition-all flex items-center gap-1.5 cursor-pointer ${
              typeFilter===chip.id
                ? 'bg-[var(--primary)] text-white shadow-sm'
                : 'bg-white border border-[var(--line-soft)] text-[var(--ink-70)] hover:border-[var(--line)]'
            }`}
          >
            {chip.label}
            <span className={`px-1.5 py-0.2 rounded-full text-[9.5px] ${typeFilter===chip.id?'bg-white/20 text-white':'bg-slate-100 text-slate-600'}`}>
              {chip.count}
            </span>
          </button>
        ))}
...
function Events({events, onSelectEvent}:{events:ManagedEvent[]; onSelectEvent:(id:string)=>void}){
  const ongoing = events.filter(e => calculateEventStatus(e) === 'ongoing');
  const upcomingEvents = events.filter(e => calculateEventStatus(e) === 'upcoming').sort((a,b) => localDate(a.event_date).getTime() - localDate(b.event_date).getTime());
  const completed = events.filter(e => calculateEventStatus(e) === 'completed').sort((a,b) => localDate(b.event_date).getTime() - localDate(a.event_date).getTime());
    ...
    <main className="flex-1 overflow-y-auto">
      <div className="px-8 py-6 max-w-[1400px] space-y-8">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-[22px] font-bold">Event Hub Workspace</h1>
            <p className="text-xs text-[var(--ink-45)] mt-1">Centralized corporate event archives, preparation assets, media & links.</p>
          </div>
        </div>
        ...
        {ongoing.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {ongoing.map(e => <EventCard key={e.id} event={e} onClick={() => onSelectEvent(e.id)} />)}
            </div>
          </section>
        )}

        <section>
          <h2 className="section-heading mb-4">Upcoming Events ({upcomingEvents.length})</h2>
          {upcomingEvents.length ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {upcomingEvents.map(e => <EventCard key={e.id} event={e} onClick={() => onSelectEvent(e.id)} />)}
            </div>
          ) : (
            <div className="py-8 text-[13px] text-[var(--ink-45)] bg-white rounded-xl border border-[var(--line-soft)] text-center">No upcoming events scheduled.</div>
          )}
        </section>

        {completed.length > 0 && (
          <section>
            <h2 className="section-heading mb-4">Completed Events ({completed.length})</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 opacity-90">
              {completed.map(e => <EventCard key={e.id} event={e} onClick={() => onSelectEvent(e.id)} />)}
            </div>
          </section>
        )}
      </div>
  ...
  return (
    <aside className={(collapsed?'w-[64px]':'w-[236px]')+' flex-shrink-0 flex flex-col bg-white border-r border-[var(--line)] transition-all duration-200'}>
      <div className={'h-16 flex items-center '+(collapsed?'justify-center px-2':'justify-between px-4')+' border-b border-[var(--line-soft)]'}>
        ...
        <button onClick={()=>setCollapsed(!collapsed)} aria-label={collapsed?'Expand sidebar':'Collapse sidebar'} title={collapsed?'Expand sidebar':'Collapse sidebar'} className="w-8 h-8 rounded-md flex items-center justify-center text-[var(--ink-45)] hover:bg-[var(--canvas-deep)]">
...
export default function LiveApp(){
  ...
  useEffect(()=>{
    if(window.location.hash.includes('reset-password')){setAuthMode('reset');setShowAuthModal(true)}
    const loadResources=()=>getManagedResources().then(res=>{setResources(res);setDbError(null)}).catch((err)=>{setResources([]);const msg=String(err?.message||err);if(msg.includes('permission denied')){setDbError('Supabase Database Permission Error: Table access is denied for the anon role.')}});
    ...
    const{data:authListener}=supabase.auth.onAuthStateChange((event)=>{
      ...
      loadResources();
      loadEvents();
      if(event==='PASSWORD_RECOVERY'){setAuthMode('reset');setShowAuthModal(true)}
    });

    window.addEventListener('vault-resources-changed',loadResources);
    return()=>{authListener.subscription.unsubscribe();window.removeEventListener('vault-resources-changed',loadResources)}
  },[]);

  const isAdvanced = profile ? (profile.role === 'advanced' || profile.role === 'teammate') && profile.status === 'approved' : false;

  useEffect(()=>{
    if(view.kind==='admin'&&!isAdmin&&!isAdvanced)setView({kind:'home'})
  },[view,isAdmin,isAdvanced]);

  const product=view.kind==='product'?products.find(p=>p.slug===view.slug):undefined;
  const selectedEvent = view.kind === 'event-detail' ? events.find(e => e.id === view.id) : undefined;
  ...
  if(!profile){
    return <div className="flex h-screen w-screen items-center justify-center bg-[var(--canvas)] overflow-hidden relative"><AuthScreen initialMode={authMode} onSuccess={()=>{fetchProfile();setView({kind:'home'})}}/></div>
  }

  return (
    <div className="flex h-screen bg-[var(--canvas)] overflow-hidden">
      <Sidebar view={view} onView={setView} isAdmin={isAdmin} profile={profile} onSignOut={async()=>{await signOut();setProfile(null);setIsAdmin(false);setView({kind:'home'})}} onOpenAuth={(mode)=>{setAuthMode(mode);setShowAuthModal(true)}}/>
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {dbError&&<div className="bg-amber-500/10 border-b border-amber-500/30 text-amber-900 px-6 py-2.5 text-[12px] flex items-center justify-between font-medium z-50 flex-shrink-0"><span>⚠️ {dbError} Run the SQL script below in your Supabase SQL Editor to grant table access.</span><a href="https://supabase.com/dashboard/project/ikkyziyugrnkolqnrxfo/sql/new" target="_blank" rel="noreferrer" className="underline font-semibold ml-4 hover:text-amber-950">Open Supabase SQL Editor</a></div>}
        <div className="flex-1 flex min-h-0 overflow-hidden">
          {view.kind==='home'?(
            <Home resources={resources} events={events} onProduct={slug=>setView({kind:'product',slug})} onSheshi={()=>setView({kind:'sheshi'})} onSelectEvent={id => setView({ kind: 'event-detail', id })}/>
          ):view.kind==='sheshi'?(
            <SheshiPage resources={resources}/>
          ):view.kind==='product'&&product?(
            <ProductPage product={product} resources={resources}/>
          ):view.kind==='all'?(
            ...
            <EventPage
              ...
              onEventUpdated={() => getEvents().then(setEvents)}
              onOpenEditModal={() => setView({ kind: 'admin' })}
            />
          ):view.kind==='admin'&&(isAdmin||isAdvanced)?(
            <AdminConsole/>
          ):(
            <Home resources={resources} events={events} onProduct={slug=>setView({kind:'product',slug})} onSheshi={()=>setView({kind:'sheshi'})} onSelectEvent={id => setView({ kind: 'event-detail', id })}/>
          )}
      ...
      {showAuthModal&&<AuthScreen initialMode={authMode} onSuccess={()=>{setShowAuthModal(false);fetchProfile()}}/>}
    </div>
  )
}