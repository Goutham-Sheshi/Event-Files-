...
const emptyForm = (): EventInput => ({ title: '', description: '', event_date: defaultEventDate(), location: '', product_id: null, event_type: 'In-person', banner: '' })
...
export default function AdminEvents({ onChanged }: { onChanged?: () => void }) {
  ...
  const save = async (e: FormEvent) => {
    ...
    try {
      const uploadedBanner = bannerFile ? await uploadEventBanner(bannerFile) : form.banner
      const payload: EventInput = { ...form, event_date: new Date(`${form.event_date}T12:00:00`).toISOString(), description: form.description || null, location: form.location || null, banner: uploadedBanner || null }
