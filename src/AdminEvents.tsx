...
import { createEvent, deleteEvent, getEvents, isCurrentUserAdmin, updateEvent, uploadEventBanner, type EventInput, type ManagedEvent } from './eventsApi'
...
export default function AdminEvents({ onChanged }: { onChanged?: () => void }) {
  ...
  const onSubmit = async (data: EventFormData) => {
    ...
    try {
      ...
      const uploadedBanner = bannerFile ? await uploadEventBanner(bannerFile) : existingBanner
      ...
      <div className="px-8 py-6 max-w-[1400px]">
        <div className="flex items-start justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-[13px] text-[var(--ink-45)] mt-1">Manage upcoming and past events with React Hook Form + Yup validation.</p>
          </div>
        ...
        {error && <div className="mb-4 px-3 py-2 rounded-lg bg-red-50 text-red-700 text-[12px]">⚠️ {error}</div>}
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {events.map(event => {
                  ...
                  const upcoming = new Date(event.event_date).getTime() >= Date.now()
                  return (
                    <tr key={event.id} className="border-b border-[var(--line-soft)] last:border-0 text-[12.5px]">
                      <td className="px-4 py-3 font-semibold text-[var(--ink)]">{event.title}</td>
                      <td className="px-4 py-3 text-[var(--ink-70)]">{product?.name || 'General'}</td>
                      <td className="px-4 py-3 text-[var(--ink-70)]">{new Date(event.event_date).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}</td>
                      <td className="px-4 py-3 text-[var(--ink-70)]">{event.location || '—'}</td>
                      <td className="px-4 py-3 text-[var(--ink-70)]">{event.event_type}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-1 rounded-full text-[10px] font-semibold ${upcoming ? 'bg-blue-50 text-blue-700' : 'bg-[var(--canvas-deep)] text-[var(--ink-45)]'}`}>
                          {upcoming ? 'Upcoming' : 'Past'}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-right whitespace-nowrap">
                        <button onClick={() => startEdit(event)} className="text-[var(--primary)] font-semibold mr-3">Edit</button>
                        <button disabled={busy} onClick={() => remove(event)} className="text-red-600 font-semibold disabled:opacity-40">Delete</button>
                      </td>
                    </tr>
                  )
                })}
        ...
        {open && (
          <div className="fixed inset-0 z-50 bg-black/35 flex items-center justify-center p-4">
            <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-xl bg-white rounded-2xl shadow-2xl p-6 max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-5">
                <div>
                  <h2 className="font-display text-[18px] font-bold">{editing ? 'Edit Event' : 'Add Event'}</h2>
                  <p className="text-[12px] text-[var(--ink-45)] mt-1">Validated with React Hook Form & Yup schema.</p>
                </div>
                <button type="button" onClick={() => setOpen(false)} className="text-[var(--ink-45)] text-xl">×</button>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <label className="sm:col-span-2 text-[12px] font-medium">
                  ...
                  />
                  {errors.title && <span className="block text-[11px] text-red-600 mt-1">⚠️ {errors.title.message}</span>}
                </label>
                ...
                <label className="text-[12px] font-medium">
                  ...
                  />
                  {errors.event_date && <span className="block text-[11px] text-red-600 mt-1">⚠️ {errors.event_date.message}</span>}
                </label>
                ...
                <label className="sm:col-span-2 text-[12px] font-medium">
                  ...
                  <span className="block text-[10px] text-[var(--ink-45)] mt-1">
                    {bannerFile ? bannerFile.name : editing?.banner ? 'Current cover image will be kept unless replaced.' : 'Upload cover banner image.'}
                  </span>
                </label>
              ...
              <div className="flex justify-end gap-2 mt-6">
                <button type="button" onClick={() => setOpen(false)} className="px-4 py-2 text-[12px] font-semibold">Cancel</button>
                <button disabled={busy} className="px-4 py-2 rounded-lg bg-[var(--primary)] text-white text-[12px] font-semibold disabled:opacity-50">
