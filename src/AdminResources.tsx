...
export default function AdminResources() {
  ...
  const grouped = useMemo(() => products.map(product => ({
    product,
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        await uploadResource({
          title: files.length === 1 ? (title.trim() || file.name) : file.name.replace(/\.[^.]+$/, ''),
  ...
  return <div className="px-8 py-6 max-w-[1400px]">
    <div className="mb-6"><h1 className="font-display text-[22px] font-bold text-[var(--ink)] tracking-tight">Related Products & Files</h1><p className="text-[13px] text-[var(--ink-45)] mt-1">Add files directly to the product they belong to. Files are stored by product, resource type and month.</p></div>

    <div className="grid grid-cols-1 xl:grid-cols-[390px_minmax(0,1fr)] gap-6 items-start">
      <form onSubmit={save} className="bg-white border border-[var(--line-soft)] rounded-2xl p-5 sticky top-4">
        <div className="flex items-center justify-between mb-4"><h2 className="font-semibold text-[14px]">Add files</h2><span className="text-[10px] font-mono text-[var(--ink-45)]">PRODUCT LIBRARY</span></div>
        <div className="space-y-3">
          <label className="block text-[12px] font-medium">Related Product<select value={productId} onChange={e=>setProductId(e.target.value)} className="mt-1.5 w-full px-3 py-2.5 rounded-lg border border-[var(--line-soft)] bg-white outline-none">{products.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
          <label className="block text-[12px] font-medium">File Type<select value={type} onChange={e=>setType(e.target.value as ResourceType)} className="mt-1.5 w-full px-3 py-2.5 rounded-lg border border-[var(--line-soft)] bg-white outline-none">{TYPES.map(t=><option key={t.value} value={t.value}>{t.label}</option>)}</select></label>
          <label className="block text-[12px] font-medium">Display Name<input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Auto-filled for one file" className="mt-1.5 w-full px-3 py-2.5 rounded-lg border border-[var(--line-soft)] outline-none"/></label>
          <label className="block text-[12px] font-medium">Tags<input value={tags} onChange={e=>setTags(e.target.value)} placeholder="logo, sales, 2026" className="mt-1.5 w-full px-3 py-2.5 rounded-lg border border-[var(--line-soft)] outline-none"/></label>
          <label className="block text-[12px] font-medium">Description<textarea value={description} onChange={e=>setDescription(e.target.value)} rows={2} placeholder="Optional context" className="mt-1.5 w-full px-3 py-2.5 rounded-lg border border-[var(--line-soft)] outline-none resize-none"/></label>
      ...
      <div className="space-y-5">{grouped.map(({ product, items: productItems }) => <section key={product.id} className="bg-white border border-[var(--line-soft)] rounded-2xl overflow-hidden">
        ...
        {productItems.length === 0 ? <div className="px-5 py-7 text-[12px] text-[var(--ink-45)]">No files added yet.</div> : <div className="divide-y divide-[var(--line-soft)]">{productItems.map(item => <div key={item.id} className="px-5 py-3.5 flex items-center gap-3"><div className="w-9 h-9 rounded-lg bg-[var(--canvas-deep)] flex items-center justify-center text-[11px] font-bold text-[var(--ink-45)]">{item.fileFormat || 'FILE'}</div><div className="min-w-0 flex-1"><div className="text-[12.5px] font-semibold truncate">{item.title}</div><div className="text-[11px] text-[var(--ink-45)] mt-0.5">{item.type} · {item.fileSize || '—'}{item.tags?.length ? ` · ${item.tags.join(', ')}` : ''}</div></div><a href={item.sourceUrl} target="_blank" rel="noreferrer" className="text-[11px] font-semibold text-[var(--primary)]">Open</a><button disabled={busy} onClick={()=>remove(item)} className="text-[11px] font-semibold text-red-600 disabled:opacity-40">Delete</button></div>)}</div>}
      </section>)}</div>
    </div>
  </div>
}
