...
export default function AdminResources() {
  ...
  const [productId, setProductId] = useState(products[0]?.id || SHESHI_ID)
  ...
  useEffect(() => { load() }, [])
  const grouped = useMemo(() => [{ product: { id: SHESHI_ID, name: 'Sheshi', color: '#E05A1C', light: '#FCE7DB' }, items: items.filter(item => item.productId === SHESHI_ID) }, ...products.map(product => ({ product, items: items.filter(item => item.productId === product.id) }))], [items])
